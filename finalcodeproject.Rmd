...
class(X2015AADT)
## make graph of highest volume days...
## make graph of the days of AADT
## makegraph of busiest routes
...
```{r}
##maped a base map by county in order to use it for future plots
...
rangetable <- zipmin %>% join(zipAVG, by="County")
range_final <- rangetable %>% join(zipmax, by="County")
...
for(i in unique(RS_df_final$ZipCode)) {
  count(total=n(STATIONS))
  } return(total)
...
```{r}
vmtbox <- ggplot(zipemissions)+ geom_histogram(aes(x=zipemissions$popden))
vmtbox
  ...
  theme(
    axis.line = element_line(color = "black"),
    axis.ticks = element_line(color = "black")) +
  labs(title="Average CO2 vs. Range of Vehicle Miles Traveled by California County",
       x="Average CO2 from Transportation (Tons/Year)",
       y="Vehicle Miles Traveled (Miles/Year)") ##second plot
...
```{r, eval=TRUE}
VMTby_county <- ggplot(data=zipemissions, aes(x=`Vehiclemilestraveled`)) + 
  geom_histogram(binwidth = 500) + geom_abline(aes(slope=1, intercept=mean(zipemissions$Vehiclemilestraveled)))
...
density_map_AADT <- ggplot() + 
  ...
  geom_point(data = aadtshp, size = .01, aes(x=aadtshp$Lon_S_or_W, y=aadtshp$Lat_S_or_W, color=factor(aadtshp$Back_AADT)), show.legend=FALSE) + theme_classic() + labs(title="Annual Average Daily Traffic for Highways in California")
...
VMTmap <- CA_base + stat_density_2d(aes(x = logs, y = lats, fill = ..level.., alpha = ..level.., size = 4, data=zipAVG, geom=polygon))
