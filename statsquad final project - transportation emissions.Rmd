...
```{r}
lastops <- read.table("/Users/alisanford/Desktop/statsfinalproject/lastops.txt", fill=TRUE, header=TRUE, quote="", sep=",") #imported data that includes names and coordinates of every bus and rail stop within Los Angeles
...
```{r}
teststops <- lastop_coords[1:50, ] #created df of 50 out of 14,514 cases to test our code without having to wait for the distance function to run through the full dfs

testzips <- emissionsdataLA[1:5, ] #created df of 5 out of 270 cases to test our code without having to wait for the distance function to run through the full dfs

```

```{r}

  for (i in 1:nrow(testzips)) { 
  d = 0
  a <- testzips$Longitude[i]
  b <- testzips$Latitude[i]
  dist <- c()
    for (j in 1:nrow(teststops)) {
    
    dist[j] = distHaversine(c(a, b),
                          c(teststops$stop_lon[j], teststops$stop_lat[j]))

  
    if (dist[j] < 4828) {
    d = d + 1
  }
    testzips$closestops[i] = d
  }
  }
...
```{r}
for (i in 1:nrow(emissionsdataSF)) {
  d = 0
  a <- emissionsdataSF$Longitude[i]
  b <- emissionsdataSF$Latitude[i]
  dist <- c()
  
    for (j in 1:nrow(SFSTOPDATA)) {
      e <- SFSTOPDATA$stop_lon[j]
      f <- SFSTOPDATA$stop_lat[j]
      dist[j] = distHaversine(c(a, b),
                              c(e, f))
      ...
      emissionsdataSF$closestops[i] = d
    }
}
