...
for (i in 1:nrow(emissionsdataLA)) {
  dLA = 0
  aLA <- emissionsdataLA$Longitude[i] 
  bLA <- emissionsdataLA$Latitude[i] 
  distLA <- c()
  
    for (j in 1:nrow(allLAstop_coords1)) {
      eLA <- allLAstop_coords1$stop_lon[j]
      fLA <- allLAstop_coords1$stop_lat[j]
      dist[j] = distHaversine(c(aLA, bLA),
                              c(eLA, fLA))
      
      if (dist[j] < 4828) {
        dLA = dLA + 1
      }
      emissionsdataLA$closestops[i] = dLA
    }
}
```
...
for (i in 1:nrow(emissionsdataSF)) {
  ...
  dist <- c()
    ...
    for (j in 1:nrow(allSFstop_coords1)) {
      c <- allSFstop_coords1$stop_lon[j]
      d <- allSFstop_coords1$stop_lat[j]
