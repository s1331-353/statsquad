...
```{r}
sfplot <- emissionsdataSF1 %>%
  ggplot(aes(x = within1km, y = `Transport(tCO2e/yr)`)) + geom_point() + stat_smooth(method = "lm")
sfplot
```

```{r}
laplot <- emissionsdataLA1 %>%
  ggplot(aes(x = within1km, y = `Transport(tCO2e/yr)`)) + geom_point() + stat_smooth(method = "lm")
laplot
```
...
```{r}
laoilplot <- emissionsdataLA1 %>%
  ggplot(aes(x = within1km, y = `FUELOIL(gallons)`)) + geom_point() + stat_smooth(method = "lm")
laoilplot
